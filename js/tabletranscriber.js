...
function quitEdition(){
	...
	jQuery.sheet.instance[count].evt.cellEditAbandon(); //Event to exit celledit
}
	...
	$("#viewer-container").iviewer(
			{
				...
				onStartLoad: function(){ $("#throbber").show(); spinner.start(); },
				onFinishLoad: function(){spinner.stop(); $("#throbber").hide();}
			});
...
function submitTask(complete, answer){
	...
	answer = {"complete" : complete, "table" : table};
